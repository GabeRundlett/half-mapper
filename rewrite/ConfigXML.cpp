...
ConfigXML::ConfigXML() {
    ...
    this->m_szGamePaths.emplace_back(HALFLIFE_DEFAULT_GAMEPATH);
    this->m_szGamePaths.emplace_back(CSTRIKE_DEFAULT_GAMEPATH);
 ...
 */
auto ConfigXML::LoadProgramConfig() -> XMLError {
    XMLError const eRetCode = this->m_xmlProgramConfig.LoadFile("config.xml");
...
auto ConfigXML::LoadMapConfig(const char *szFilename) -> XMLError {
    ...
    while (wad != nullptr) {
        this->m_vWads.emplace_back(wad->GetText());
    ...
    while (chapter != nullptr) {
        ...
        if (sChapterEntry.m_szName.empty()) {
 ...
 */
auto ConfigXML::WriteDefaultProgramConfig() -> XMLError {
    // Document root node.
    ...
    // Save.
    XMLError const eRetCode = this->m_xmlProgramConfig.SaveFile("config.xml");
