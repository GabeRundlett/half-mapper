...
BSP::BSP(const std::vector<std::string> &szGamePaths, const string &filename, const MapEntry &sMapEntry) {
    string const id = sMapEntry.m_szName;
    ...
    for (const auto &szGamePath : szGamePaths) {
        if (!inBSP.is_open()) {
            inBSP.open(szGamePath + filename, ios::binary);
        }
    }

    // If the BSP wasn't found in any of the gamepaths...
    if (!inBSP.is_open()) {
        cerr << "Can't open BSP " << filename << "." << endl;
        return;
    }
    ...
    if (bHeader.nVersion != 30) {
        cerr << "BSP version is not 30 (" << filename << ")." << endl;
        return;
    }
    ...
    for (int i = 0; i < bHeader.lump[LUMP_FACES].nLength / (int)sizeof(BSPFACE); i++) {
        ...
        string const faceTexName = texNames[b.iMiptex];
        ...
        for (int j = 2, k = 1; j < f.nEdges; j++, k++) {
            ...
            minUV[i * 2] = min(minUV[i * 2], c1.u);
            minUV[i * 2 + 1] = min(minUV[i * 2 + 1], c1.v);
            minUV[i * 2] = min(minUV[i * 2], c2.u);
            minUV[i * 2 + 1] = min(minUV[i * 2 + 1], c2.v);
            minUV[i * 2] = min(minUV[i * 2], c3.u);
            minUV[i * 2 + 1] = min(minUV[i * 2 + 1], c3.v);

            maxUV[i * 2] = max(maxUV[i * 2], c1.u);
            maxUV[i * 2 + 1] = max(maxUV[i * 2 + 1], c1.v);
            maxUV[i * 2] = max(maxUV[i * 2], c2.u);
            maxUV[i * 2 + 1] = max(maxUV[i * 2 + 1], c2.v);
            maxUV[i * 2] = max(maxUV[i * 2], c3.u);
            maxUV[i * 2 + 1] = max(maxUV[i * 2 + 1], c3.v);
        }
...
#define LMXY(_x, _y) (((_x) + ((_y)*lmaps[i].w)) * 3)
    ...
    for (auto it = texturedTris.begin(); it != texturedTris.end(); it++, i++) {
        ...
        glBufferData(GL_ARRAY_BUFFER, (*it).second.triangles.size() * sizeof(VECFINAL), (void *)(*it).second.triangles.data(), GL_STATIC_DRAW);
...
void BSP::render() {
    ...
    for (auto it = texturedTris.begin(); it != texturedTris.end(); it++, i++) {
        ...
        if ((*it).first != "aaatrigger" && (*it).first != "origin" && (*it).first != "clip" && (*it).first != "sky" && (*it).first[0] != '{' && !(*it).second.triangles.empty()) {
            ...
            glTexCoordPointer(2, GL_FLOAT, sizeof(VECFINAL), (char *)nullptr + 4 * 3);
            glEnableClientState(GL_TEXTURE_COORD_ARRAY);

            ////T1
            glClientActiveTextureARB(GL_TEXTURE1_ARB);
            glTexCoordPointer(2, GL_FLOAT, sizeof(VECFINAL), (char *)nullptr + 4 * 5);

            glVertexPointer(3, GL_FLOAT, sizeof(VECFINAL), (void *)nullptr);
